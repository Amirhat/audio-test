...
export default () => {
    ...
    const normalizeData = filteredData => {
        const multiplier = Math.pow(Math.max(...filteredData.map(i => i.max), ...filteredData.map(i => -1 * i.min)) ,-1);
        return filteredData.map(n => {return {min: n.min * multiplier , max: n.max * multiplier}});
    }
    ...
    const draw2 = (ctx, canvas, normalizedData, start, end, color= "#ffffff") => {
        // Set up the canvas
        ...
        // clear old lines
        ctx.clearRect((start - .5) * width, -1 * canvas.offsetHeight, (end - start) * width, 2 * canvas.offsetHeight);
